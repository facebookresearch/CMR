...
class MemoryBasedCL(ContinualFinetuning):
    ...
    def get_top_interfered_examples(self, K, candidate_examples, query_data_loader):
        ...
        for item in candidate_examples:
            candidate_examples_single_ans.append((item[0], item[1][0:1], item[2]))
        # self.logger.info(f"candidate_examples: {candidate_examples}")
        ...
        for example, before_loss, after_loss in zip(candidate_examples, before_losses, after_losses):
            if self.debugger_args.mir_abalation_args == "none":
                loss_delta = after_loss - before_loss
            elif self.debugger_args.mir_abalation_args == "largest_afterloss":
    ...
    def local_adaptation(self, model, adapt_dataloader, diff_loss_weight=1e-3):
