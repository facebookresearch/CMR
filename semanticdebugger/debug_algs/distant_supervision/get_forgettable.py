...
if __name__ == '__main__':
    args = run_lifelong_finetune.get_cli_parser().parse_args()
    ...
    with open("exp_results/supervision_data/error_forget_pairs.json", "w") as f:
        json.dump(all_mined_supervision, f)


"""
prefix=nq_dev_0812_wr_mined_supervision_from_train
log_file=exp_results/supervision_data/run_${prefix}.log
echo ${log_file}
touch ${log_file}
CUDA_VISIBLE_DEVICES=0,1,2,3 python semanticdebugger/debug_algs/distant_supervision/get_forgettable.py \
    --cl_method_name simple_cl_for_mining_supervision \
    --learning_rate 3e-5 --num_train_epochs 5 --train_batch_size 16 \
    --prefix ${prefix} \
    --stream_mode dynamic \
