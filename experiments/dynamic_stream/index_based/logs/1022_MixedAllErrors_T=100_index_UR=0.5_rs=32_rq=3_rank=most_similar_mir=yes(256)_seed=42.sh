...
prefix="1022_MixedAllErrors_T=${max_timecode}_index_UR=${upstream_ratio}_rs=${replay_size}_rq=${replay_freq}_rank=${rank_method}_mir=${use_mir}(${mir_cand_size})_seed=${seed}"
...
tmp_script_copy=exp_results/dynamic_stream/index_based/logs/${prefix}.sh
...
cp exp_results/dynamic_stream/index_based/run_bart_index.sh $tmp_script_copy
...
CUDA_VISIBLE_DEVICES=$gpu python semanticdebugger/debug_algs/run_lifelong_finetune.py \
    ...
    --sampled_upstream_json_path data/mrqa_naturalquestions/mrqa_naturalquestions_train.jsonl \
    ...
    --save_all_ckpts 0 \
    --init_memory_cache_path "exp_results/data_streams/bart_index.upstream_memory.full.pkl" \
